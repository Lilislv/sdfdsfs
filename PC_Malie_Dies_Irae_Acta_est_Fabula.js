...
const sendLine = trans.send((s) => s);
let lastLine = '';
let lastLineAt = 0;

function handlerLine(payload) {
    const text = String(payload || '');
    const now = Date.now();
    if (!text) return;
    if (text === lastLine && now - lastLineAt < 250) return;
    lastLine = text;
    lastLineAt = now;
    sendLine(text);
}
...
(function () {
    ...
    Breakpoint.add(hookAddress, function () {
        ...
        const payload = readString(strBuffer.add(strPointer));
        handlerLine(payload.text);
    })
})();
                ...
                if (cmd === 0x1) {
                    ...
                    while ((c = address.readU16()) !== 0) {
                        ...
                        if (c === 0xa) {
                            ...
                            if (surface && rubi) {
                                furigana.push({ text: surface, reading: rubi });
                                s += '（' + rubi + '）';
                            }
                            break;
                        }
